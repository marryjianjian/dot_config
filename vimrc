...
" Formatting {
    ...
    autocmd FileType c,cpp,java,go,php,javascript,puppet,python,rust,twig,xml,yml,perl,sql,lua autocmd BufWritePre <buffer> if !exists('g:spf13_keep_trailing_whitespace') | call StripTrailingWhitespace() | endif
    "autocmd FileType go autocmd BufWritePre <buffer> Fmt
    autocmd BufNewFile,BufRead *.html.twig set filetype=html.twig
    autocmd FileType haskell,puppet,ruby,yml,lua setlocal expandtab shiftwidth=2 softtabstop=2
...
" Plugins {
    ...
    " GoLang {
        if exists('g:spf13_bundle_groups') && count(g:spf13_bundle_groups, 'go')
    ...
    " Ctags {
        if isdirectory(expand("~/.vim/plugged/vim-gutentags"))
            ...
            let g:gutentags_ctags_extra_args += ['--kinds-c=+pxL']
            let g:gutentags_ctags_extra_args += ['--kinds-go=+R']
            let g:gutentags_ctags_exclude = ['.git', 'vendor', 'build*', 'cmake-build-*']
    ...
    " PyMode {
    " https://github.com/vim/vim/issues/3117 did not work for me ,
    " so just do not use pymode
        let g:pymode = 0
    ...
    " Ack.vim {
        if isdirectory(expand("~/.vim/plugged/ack.vim/")) && executable("rg")
            ...
            command! -nargs=1 Ag execute "Ack! -g '!{**/tests,**/data}' <args> " . Find_git_root()
            cnoreabbrev Ack Ack!
